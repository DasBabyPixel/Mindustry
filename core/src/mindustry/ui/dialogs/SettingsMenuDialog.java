...
public class SettingsMenuDialog extends BaseDialog{
    ...
    void addSettings(){
        ...
        if(!mobile){
            ...
            graphics.checkPref("fullscreen", false, b -> platform.updateFullscreen());
            graphics.checkPref("borderlesswindow", true, b -> platform.updateFullscreen());

            Core.graphics.setVSync(Core.settings.getBool("vsync"));

            if(Core.settings.getBool("fullscreen")){
                // also handles borderlesswindow setting
                Core.app.post(() -> platform.updateFullscreen());
            }
        }else if(!ios){
